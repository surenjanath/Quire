...
- **Go (⌘K)**: A small sheet for commands (Chat, Claude Code, New, Settings…) and a search of past pages. Journal → Go. Type a word from an old page to jump there. Random page is in the list. Typing a real question (4+ characters, matching no command) also offers "Ask: '...'" — picking it asks the offline Ollama model that question, grounded in up to 6 pages from the *whole* journal (`JournalContext.related`, no current-page anchor), not just today's page or its neighbors. Opens nothing if nothing matches (`askJournal`), rather than let the model guess; the system prompt (`PromptLibrary.defaultAskJournalPrompt`) tells it to say plainly when the journal doesn't say. Uses the same Ollama panel as everything else, so Copy/Insert/Replace/Note and follow-ups all work identically.
